...
#set page(
  paper: "a4",
  margin: (top: 1cm, bottom: 0.7cm, left: 1cm, right: 1cm),
  header: none,
  footer: none,
)
...
#grid(
  ...
  [
    #image("/template/assets/images/uftlabfis-logo.png", width: 1.5cm)
  ]
)
...
#cetz.canvas(length: 1mm, {
  ...
  for i in range(y+1) {
    ...
    if i <= x {
      ...
      if calc.rem(i, 50)==0 and i != 0 {
        content((i, -1mm), [#i], anchor: "north")
        content((i, y+1), [#i], anchor: "south")
      }
    }
    ...
    if calc.rem(i, 50)==0 and i != 0 {
      content((-1mm, i), [#i], anchor: "east")
      content((x+1, i), [#i], anchor: "west")
    }
